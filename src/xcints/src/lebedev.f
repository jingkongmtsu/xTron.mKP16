       subroutine lebedev(X,Y,Z,W,NPT)
       implicit none
       double precision X(*),Y(*),Z(*),W(*)
       integer NPT
       integer N
C
C      This program is the top interface function to use lebedev grids.       
C      The original code is taken from CCL:
C      http://www.ccl.net/cca/software/SOURCES/FORTRAN/Lebedev-Laikov-Grids/index.shtml
C      see the code below.
C
C      input:
C      npt    number of grid points specified by the user
C      output:
C      x,y,z: coordinate components for the points
C      w    : weights for each point
C      
       N = 0 
       select case(NPT)
          case(6)
...
C      fenglai: we disable the stop below
C      this is never called
c       stop 
    1  continue
       a=1.0d0
       x(1) =  a
